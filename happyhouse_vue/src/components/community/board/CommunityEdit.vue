  ...
  <div>
	<div id="container" class="textCenter">
        <div id="guide" style="display:flex; align-items: flex-end"> 
            <div style="font-size: 22px; margin-right: 5px">
                Community
            </div>
        ...
        <div style="display: flex; justify-content: center">
            <div class="contentBox contentLeft">
                <input id="title" class="inputedText" placeholder="글 제목을 입력해 주세요" v-model="title">
                <textarea id="mainContent" class="inputedText" v-model="content" placeholder="글 내용을 입력해 주세요
...
"></textarea>
            ...
            <div class="contentBox contentRight">
                ...
                <b-button variant="outline-primary" @click="upload()">수정하기</b-button>
            </div>
        </div>
	</div>
  </div>
...
export default {
	...
	methods: {
        async upload() {
			// fileChange가 true라면 폴더 비우고
			
            await axios.post("http://localhost:9999/rest/board/edit", {
				...
				if (this.fileChange == true) {
					// 폴더 비우기
					// 새로 채우기하는데 만약 this.file.length == 0 이면 리턴 아니면 폴더 채우러 통신
					axios.get("http://localhost:9999/rest/file/clearFolder/"+this.$route.params.articleNo)
					.then(() => {
						alert('ttt')
						...
						axios.post("http://localhost:9999/rest/file/articleFile/"+this.$route.params.articleNo, formData, {headers: {'Content-Type': 'multipart/form-data'}})
	...
	async created() {
		...
		// 폴더 체크하고 폴더있으면 내용물 몇개인지 리턴
		//              폴더없으면             0 리턴
		// 1부터 리턴 한 만큼 돌면서 loadedImage.push(require(@/article/글번호/i))
		var len = await axios.get("http://localhost:9999/rest/file/folderChk/" + this.$route.params.articleNo).then(({data}) => data)
		console.log(len)
		for (var i = 1; i <= len; i++) {
			this.loadedImage.push(require("@/article/" + this.$route.params.articleNo + "/" + i + ".png"));
		}
